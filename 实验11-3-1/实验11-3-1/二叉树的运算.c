...
/*层次遍历二叉树算法思想：采用一个队列q，先将二叉树根结点入队列，然后退队*/
/*列，输出该结点；若它有左子树，便将左子树根结点入队列；若它有右子树，便将*/
/*右子树根结点入队列，直到队列空为止。因为队列的特点是先进先出，从而达到按*/
/*层次顺序遍历二叉树的目的*/
...
void prtbtree(BiTree bt, int level)/*逆时针90度输出二叉树树形*/
{
	int j;
	if (bt)
	{
...
void exchange(BiTree bt)/*交换二叉树左右子上树*/
{
	BiTree p;
	if (bt)
	{
